...
function isTagalog(text) {
  const tagalogWords = ['ang', 'ng', 'na', 'sa', 'at', 'ay', 'ko', 'mo', 'ka', 'po', 'opo',
    ...
    'yun', 'doon', 'dito', 'nito', 'nito', 'kanila', 'kanyang', 'pag', 'mag', 'mga'];
  const words = text.toLowerCase().split(/\s+/);
  const matches = words.filter(w => tagalogWords.includes(w));
...
async function chatAboutMusic(msg, threadID) {
  ...
  const res = await axios.post('https://text.pollinations.ai/', {
    messages: [{ role: 'system', content: MUSIC_SYSTEM }, ...h],
...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (!args.length) {
    return api.sendMessage(
      ...
      `🎤 ${bold('Mag-a-send ng voice audio message!')} 🎵`,
      threadID, messageID
    );
  }
  ...
  if (sub === 'reset') {
    ...
    return api.sendMessage(`🔄 ${bold('Conversation cleared!')}\n💬 Type ${P}music [request] para magsimula.`, threadID, messageID);
  }

  if (sub === 'chat') {
    const question = args.slice(1).join(' ').trim();
    if (!question) return api.sendMessage(`❌ Lagyan ng tanong!\n💡 ${P}music chat Ano ang OPM?`, threadID, messageID);
  ...
  try {
    const [lyrics, audioPath] = await Promise.all([
      generateLyrics(request, lang),
      generateLyrics(request, lang).then(l => textToVoice(l, lang))
    ]);
